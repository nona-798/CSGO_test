...
public class EnemyFSM : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform projectileSpawnPos;             // 발사체 생성 위치
    [SerializeField]
    private float attackRange = 5;                    // 공격 범위 (이 범위 안에 들어오면 공격샅애로 변환)
    [SerializeField]
    ...
    private IEnumerator Wander()
    {
        ...
        while(true)
        {
            ...
            // 목표 위치에 근접하게 도달하거나 너무 오랜 시간동안, 배회하기 상태에 머물러 있으면
    ...
    private Vector3 CalculateWanderPosition()
    {
        ...
        // 생성된 목표 위치가 자신의 이동구역을 벗어나지 않도록 설정
    ...
    private IEnumerator Attack()
    {
        ...
        while(true)
        {
            ...
            // 타겟과의 거리에 따라 행동 선택 (배회, 추격, 공격)
            CalculateDistanceToTargetAndSelectState();

            if(Time.time - lastAttackTime > attackRate)
            {
                // 공격 주기가 되어야 공격할 수 있도록 하기 위해 시간 저장
